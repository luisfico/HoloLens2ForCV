...
def save_single_pcloud(shared_dict,
    ...
    #output_path = str(path)[:-4] + f'{suffix}.pcd'
    output_path = str(path)[:-4] + '.pcd'  #without suffix _cam
...
#        return
    ...
    if onceGen==True : # <-------- ENABLE TO WRITE LUTF.CSV 
        ...
        print("lutLT_conversionPoints2dTo3d.csv generated! ")
...
def save_ply(output_path, points, rgb=None, cam2world_transform=None):
    ...
    #o3d.io.write_point_cloud(output_path, pcd)  #save in binary format
    o3d.io.write_point_cloud(output_path, pcd, write_ascii=True)   #save in ascci format
